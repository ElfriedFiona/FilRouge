...
const ArtisanCard = ({ artisanData }) => {
  const navigate = useNavigate();
  const { artisan } = artisanData;

  // Assurez-vous que l'objet artisan et ses propriétés existent
  ...
  const rating = reviews > 0 ? totalNotes / reviews : 0; // Par défaut à 0 si aucun avis
    ...
    <div className="bg-gray-300 shadow-md rounded-lg p-6 mb-6 flex flex-col md:flex-row justify-between md:items-start gap-4">
      <div className="flex flex-col sm:flex-row gap-6 flex-1">
        ...
        <div>
          ...
          <h3 className="text-lg text-gray-700">{professionName}</h3> {/* Affichage de la profession */}
